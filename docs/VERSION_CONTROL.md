...
| `v4.2-auto-trend-line` | V4.2 feature branch (automatic trend line drawing). Cut from `main` after v4.1.2 was merged in. Scope not yet defined at branch-creation time -- placeholder row, to be filled in once the feature is scoped. |
...
- 4 - advance "dashhboard, tools v2, intetration"
	...
	○ 4.2 (`v4.2-auto-trend-line`) automatic trend line drawing -- scope not yet defined
- 5 - cosmetic
- 6 - tax management
...
## Current status (as of `v4.2-auto-trend-line`)

`main` has V1, V2, V2.1, V2.2, V2.3, V2.4, V3, V3.1, V4, V4.1.1, V4.1,
and V4.1.2 merged in, tagged through `v4.1.2` (tip commit `5f90111`,
"Merge v4.1.2: fix Monitor Stocks crash on Streamlit Community Cloud").
`v4.1.2` fixed a production-only `AttributeError` on Monitor Stocks
found via a live-app screenshot. `v4.2-auto-trend-line` was just cut
from `main` to add automatic trend line drawing -- scope not yet
defined, nothing implemented yet. Also still open: whether to apply the
"Total Return" concept (Total P/L/%) to the Dashboard page (raised
during v4.1, not yet scoped), and confirming the redeployed
`myinvestment27.streamlit.app` (pointed at `main`, root-level
`dashboard_app.py` as main file path) has actually picked up v4.1.2.
